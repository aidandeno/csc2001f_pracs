...
public class SimpleTreeWriterImpl implements SimpleTreeWriter
{
    ...
    @Override
    /**
     * Creates and prints textual representation of a AVL Search Tree to a
     * destination set at instantiation.
     *
     * @param tree Root node of AVL Search Tree to be printed.
     */
    public void write(AVLTreeNode tree)
    {
        ...
        ArrayList<AVLTreeNode> levelList = (ArrayList<AVLTreeNode>) TreeUtils.levelZero(tree);
        ...
        for (int i = 0; i <= height; i++)
        {
            ...
            int initialSpaces  = textWidth * (int) ((Math.pow(2, height - i) - 1) / 2) - 4;
            //the number of spaces between values on a level
            int internalSpaces = textWidth * (int) (Math.pow(2, height - i) - 1) - 4;
            ...
            Iterator<AVLTreeNode> it;
            for (it = levelList.iterator(); it.hasNext();)
            {
                AVLTreeNode element = it.next();
                ...
                {
                    outputLocation.printf("%" + textWidth + "s", element);
                }
                ...
                else
                {
                    ...
                    Iterator<AVLTreeNode> it2;
                    for (it2 = levelList.iterator(); it2.hasNext();)
                    {
                        AVLTreeNode element2 = it2.next();
            ...
            levelList = (ArrayList<AVLTreeNode>) TreeUtils.nextLevel(levelList);
        }
    }
}